...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // About page — chapter timeline (2006 / Growth & Expansion / Today).
  // Only one chapter is shown at a time so the page never presents the full
  // history as a single wall of text.
  const chapterNav = document.querySelector('.chapter-nav');
  if (chapterNav) {
    const tabs = chapterNav.querySelectorAll('button[data-chapter]');
    const panels = document.querySelectorAll('.chapter-panel[data-chapter]');
    cggEach(tabs, (tab) => {
      tab.addEventListener('click', () => {
        cggEach(tabs, (t) => { t.classList.remove('active'); t.setAttribute('aria-selected', 'false'); });
        cggEach(panels, (p) => { p.classList.remove('active'); p.hidden = true; });
        tab.classList.add('active');
        tab.setAttribute('aria-selected', 'true');
        const target = document.querySelector('.chapter-panel[data-chapter="' + tab.dataset.chapter + '"]');
